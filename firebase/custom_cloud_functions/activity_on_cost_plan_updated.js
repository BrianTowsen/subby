...
// The cost plan (projects/{projectId}/estimate/plan) autosaves on a 700ms
...
// keyed cost_updated_{projectId}_{yyyymmdd}. Result: at most one "Costs
...
exports.activityOnCostPlanUpdated = functions
  .region(REGION)
  .firestore.document("projects/{projectId}/estimate/{docId}")
  .onWrite(async (change, context) => {
    ...
    const id =
      "cost_updated_" + context.params.projectId + "_" + dayKey(new Date());
    await db.collection("activity").doc(id).set(
      {
        projectRef: projectRef,
        type: "cost_updated",
        title: "", // feed renders the type label alone: "Costs updated"
        actorRef: null,
        actorName: "",
