...
/// EditTimelinePageView — Construction Programme (Gantt) TEMPLATE.
///
...
/// sub-task time.
class EditTimelinePageView extends StatefulWidget {
  const EditTimelinePageView({
    super.key,
    this.width,
    this.height,
    this.projectRef,
  ...
  @override
  State<EditTimelinePageView> createState() => _EditTimelinePageViewState();
}
...
class _EditTimelinePageViewState extends State<EditTimelinePageView> {
  ...
  static const Color _header = Color(0xFF808789); // hero header ink
  ...
  bool _ready = false; // programme loaded → safe to render the editor
  ...
  void didChangeDependencies() {
    ...
    // Standalone EDIT route — seed the node to edit from the route query.
    final _qp = GoRouterState.of(context).uri.queryParameters;
    _selSi = int.tryParse((_qp['secIndex'] ?? '').trim()) ?? 0;
    _selCi = int.tryParse((_qp['childIndex'] ?? '').trim()) ?? -1;
    // Never spin forever if the programme genuinely never loads.
  ...
  void _back() {
    _flushSave(); // ensure the last edit reaches Firestore before we leave
    context.safePop();
  }

  // Push any debounced save through immediately (called on the way out).
  void _flushSave() {
    _saveTimer?.cancel();
    _saveNow();
  }
  ...
  void _performDeleteSel() {
    ...
    _syncNameCtl();
    _flushSave();
    context.safePop(); // deleting the edited node returns to the timeline
  }
  ...
  Widget build(BuildContext context) {
    // This widget IS the edit page now (its own route). It renders only the
    // node editor; back/Done/delete pop the route and the timeline — which
    // streams the same programme doc — reflects the change on return.

    // Keep the selection in range in case the programme changed under us.
    if (_ready && _sections.isNotEmpty) {
      if (_selSi < 0 || _selSi >= _sections.length) {
        _selSi = _sections.length - 1;
        _selCi = -1;
      }
      final kids = _sections[_selSi].children;
  ...
  // Shown until the programme has loaded (avoids editing/saving the placeholder).
  Widget _editLoading() {
